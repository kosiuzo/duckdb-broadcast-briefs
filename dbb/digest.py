...
def markdown_to_html(text: str) -> str:
    """
    Convert basic markdown to HTML with support for nested lists.
    ...
    # Process lists line by line to support nesting
    lines = text.split('\n')
    result = []
    stack = []  # Stack of (indent_level, list_type)
    ...
    for line in lines:
        ...
        if numbered_match:
            ...
            while stack and stack[-1][0] > indent_level:
                list_type = stack.pop()[1]
                result.append(f'</li>\n</{list_type}>')

            # Close previous item if at same level
            if stack and stack[-1][0] == indent_level:
                result.append('</li>')
            ...
            if not stack or stack[-1][0] < indent_level:
                if stack:
                    ...
                    result.append('<ol>')
                stack.append((indent_level, 'ol'))
            elif stack and stack[-1][1] != 'ol':
                # Switch from ul to ol
                result.append('</ul>')
                result.append('<ol>')
                stack[-1] = (indent_level, 'ol')

            result.append(f'<li>{item_text}')
            ...
            if not stack or stack[-1][0] < indent_level:
                if stack:
                    ...
                    result.append('<ul>')
                stack.append((indent_level, 'ul'))
            elif stack and stack[-1][1] != 'ul':
                # Switch from ol to ul
                result.append('</ol>')
                result.append('<ul>')
                stack[-1] = (indent_level, 'ul')
...
"""
    ...
    def render_digest(self, episodes: List[Dict[str, Any]], date_range: Tuple[datetime, datetime]) -> Tuple[str, str]:
        ...
        # Template context
        context = {
            "title": f"Your Weekly Podcast Digest ({start_date.strftime('%Y-%m-%d')} – {end_date.strftime('%Y-%m-%d')})",
    ...
    def render_channel_digest(self, episodes: List[Dict[str, Any]], channel: str, date_range: Tuple[datetime, datetime]) -> Tuple[str, str]:
        """
        Render channel-specific digest HTML and plaintext.

        Args:
            episodes: List of episode dictionaries for a single channel
        ...
        # Convert markdown to HTML for each episode
        for episode in episodes:
            if episode.get("summary_md"):
                episode["summary_html"] = markdown_to_html(episode["summary_md"])
            else:
        ...
        # Template context for single channel
        context = {
            "title": f"{channel} Weekly Digest ({start_date.strftime('%Y-%m-%d')} – {end_date.strftime('%Y-%m-%d')})",
        ...
        # Render HTML using channel-specific template
        html_template = self._load_template("channel_digest.html")
        html_content = html_template.render(**context)
        ...
        # Render plaintext using channel-specific template
        text_template = self._load_template("channel_digest.txt")
        text_content = text_template.render(**context)

        logger.info(f"Channel digest rendered for '{channel}' with {len(episodes)} episode(s)")
        return html_content, text_content
...
class DigestSender:
    ...
    def send_digest(self, html_content: str, text_content: str, subject: str, recipients: List[str] = None) -> bool:
        """
        Send digest email.
        ...
        # Use provided recipients or fall back to config default
        email_recipients = recipients if recipients else self.config.email.recipients
        ...
        try:
            ...
            logger.info(f"Digest sent to {len(email_recipients)} recipient(s)")
            return True
